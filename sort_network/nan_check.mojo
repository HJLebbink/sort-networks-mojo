...
def test_float32_16():
    ...
    var elapsed_time_ns = perf_counter_ns() - start_time_ns
...
def test_intrinsic_1():
    ...
    var start_time_ns = perf_counter_ns()
    # var data2: T2 = data0.max(data1)

    # https://llvm.org/docs/LangRef.html#llvm-maximum-intrinsic
    var data2: T2 = rebind[SIMD[T, channels]](
        llvm_intrinsic["llvm.maximum", T2, T2, T2](data0, data1)
    )
...
def test_intrinsic_2():
    ...
    var data2: T2 = rebind[SIMD[T, channels]](
        llvm_intrinsic["llvm.maxnum", T2, T2, T2](data0, data1)
    )
...
def test_llvm_1():
    ...
    # var data1: SIMD[DType.float32, 16] = 20.

    var data0x = rebind[__mlir_type.`vector<16xf32>`](data0)
    var data1x = rebind[__mlir_type.`vector<16xf32>`](data1)
    ...
    var data2 = rebind[SIMD[DType.float32, 16]](tmp)
    print(data2)
    print("Elapsed time " + String(elapsed_time_ns) + " ns")
