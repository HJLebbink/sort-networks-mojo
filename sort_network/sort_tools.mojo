...
def swap_idx[
    ...
    comptime perm = gen_perm[swaps, width]()
    var CONST_data = t[0]
    var CONST_idx = t[1]
...
def linear_to_layers(linear_lst: List[SIMD[DType.uint16, 2]]) -> SwapData:
    ...
    for p in range(len(tmp)):
        ...
        for k in range(len(CONST_org)):
            var CONST_x: Int = Int(CONST_org[k][0] >> 16)
            var CONST_y: Int = Int(CONST_org[k][1] & 0xFFFF)
            new.append(SIMD[DType.uint16, 2](CONST_x, CONST_y))
        result.add_layer_l(Layer(new^))
    return result^


def layers_to_linear(sd: SwapData) -> List[SIMD[DType.uint16, 2]]:
...
def linear_print(a: List[SIMD[DType.uint16, 2]]):
    for i in range(len(a)):
        print("(" + String(a[i][0]) + "," + String(a[i][1]) + "),", end="")


def test_perm_code():
    comptime swap16 = swap_data[16]()

    comptime l1_obs = gen_perm[swap16[0], 16]()
    comptime l1_exp = IndexList[16](
        5, 4, 12, 13, 1, 0, 7, 6, 9, 8, 15, 14, 2, 3, 11, 10
    )
    constrained[l1_obs == l1_exp, "l1"]()

    comptime l2_obs = gen_perm[swap16[1], 16]()
    comptime l2_exp = IndexList[16](
        2, 10, 0, 6, 7, 14, 3, 4, 11, 12, 1, 8, 9, 15, 5, 13
    )
    constrained[l2_obs == l2_exp, "l2"]()

    comptime l3_obs = gen_perm[swap16[2], 16]()
    comptime l3_exp = IndexList[16](
        8, 3, 11, 1, 13, 9, 10, 15, 0, 5, 6, 2, 14, 4, 12, 7
    )
    constrained[l3_obs == l3_exp, "l3"]()

    comptime l4_obs = gen_perm[swap16[3], 16]()
    comptime l4_exp = IndexList[16](
        1, 0, 4, 8, 2, 6, 5, 12, 3, 10, 9, 13, 7, 11, 15, 14
    )
    constrained[l4_obs == l4_exp, "l4"]()

    comptime l5_obs = gen_perm[swap16[4], 16]()
    comptime l5_exp = IndexList[16](
        0, 3, 5, 1, 8, 2, 9, 11, 4, 6, 13, 7, 14, 10, 12, 15
    )
    constrained[l5_obs == l5_exp, "l5"]()

    comptime l6_obs = gen_perm[swap16[5], 16]()
    comptime l6_exp = IndexList[16](
        0, 2, 1, 5, 11, 3, 8, 9, 6, 7, 12, 4, 10, 14, 13, 15
    )
    constrained[l6_obs == l6_exp, "l6"]()

    comptime l7_obs = gen_perm[swap16[6], 16]()
    comptime l7_exp = IndexList[16](
        0, 1, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10, 13, 12, 14, 15
    )
    constrained[l7_obs == l7_exp, "l7"]()

    comptime l8_obs = gen_perm[swap16[7], 16]()
    comptime l8_exp = IndexList[16](
        0, 1, 2, 3, 6, 7, 4, 5, 10, 11, 8, 9, 12, 13, 14, 15
    )
    constrained[l8_obs == l8_exp, "l8"]()

    comptime l9_obs = gen_perm[swap16[8], 16]()
    comptime l9_exp = IndexList[16](
