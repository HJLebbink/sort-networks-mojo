...
fn oddEvenMergeSort[lo: Int, channels: Int](inout result: List[SIMD[DType.uint16, 2]]):
    if channels > 1:
...
fn oddEvenMerge[lo: Int, channels: Int, r: Int](inout result: List[SIMD[DType.uint16, 2]]):
    alias m: Int = r*2
    @parameter
    if m < channels:
        ...
        while ((i + r) < (lo + channels)):
            result.append(SIMD[DType.uint16, 2](i, i+r))
            i += m
    else:
        result.append(SIMD[DType.uint16, 2](lo, lo+r))
