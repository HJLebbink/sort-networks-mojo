...
def sn_ml_4n[
    ...
    var tmp3 = sort_2x[T, sd, sequential, ascending](tmp1, tmp2)
    var CONST_d02 = rebind[dest_type = SIMD[T, sub_size * 2]](tmp3[0])
    var CONST_d13 = rebind[dest_type = SIMD[T, sub_size * 2]](tmp3[1])
    ...
    var CONST_d01: SIMD[T, 2 * sub_size] = sn_merge[T, 2 * sub_size, ascending](
        d0.join(d1)
    )
    var CONST_d23: SIMD[T, 2 * sub_size] = sn_merge[T, 2 * sub_size, ascending](
        d2.join(d3)
    )
    ...
    var CONST_d12: SIMD[T, 2 * sub_size] = sn_merge[T, 2 * sub_size, ascending](
        d1.join(d2)
    )
...
def sn_ml_8n[
    ...
    if True:
        ...
        var CONST_d35: SIMD[T, 2 * sub_size] = sn_merge[
            T, 2 * sub_size, ascending
        ](d3.join(d5))
