...
from sort_network_data import swap_data, swap_data_2x, swap_data_already_sorted_32_32, swap_data_already_sorted_16_16, swap_data_already_sorted_8_8
...
@always_inline
fn sort_networ_new_CRASHES[
    T: DType, channels: Int, ascending: Bool = True
](v: SIMD[T, channels]) -> SIMD[T, channels]:
    alias sd: SwapData = swap_data[channels]()
    return sort_network_internal_CRASHES[T, channels, sd, ascending](v)


@always_inline
fn sort_network_internal_CRASHES[
...
](v: SIMD[T1, channels], idx: SIMD[T2, channels]) -> (SIMD[T1, channels], SIMD[T2, channels]):
...
@always_inline
fn sort_network_2x_interleave[
    T1: DType, T2: DType, channels: Int, ascending1: Bool = True, ascending2: Bool = True
](va: SIMD[T1, channels], vb: SIMD[T2, channels]) -> (SIMD[T1, channels], SIMD[T2, channels]):
    alias sd: SwapData = swap_data[channels]()
    alias n_layers: Int = sd.n_layers
...
@always_inline
fn sort_network_2x_parallel[
    T: DType, channels: Int, ascending: Bool = True
](va: SIMD[T, channels], vb: SIMD[T, channels]) -> (SIMD[T, channels], SIMD[T, channels]):
